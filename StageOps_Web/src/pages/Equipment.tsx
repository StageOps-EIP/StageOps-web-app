...
import { getEquipment, createEquipment, updateEquipment } from '@/services/equipment.service';
    ...
    <div className="p-8">
      ...
      <div className="mb-4">
        <p role="status" aria-live="polite" aria-atomic="true" className="text-sm text-content-muted">
          {filteredEquipment.length} équipement{filteredEquipment.length !== 1 ? 's' : ''} trouvé
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-theme-elevated border-b border-theme-border">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-content-muted uppercase tracking-wider">Équipement</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-content-muted uppercase tracking-wider">Catégorie</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-content-muted uppercase tracking-wider">QR Code</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-content-muted uppercase tracking-wider">Emplacement</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-content-muted uppercase tracking-wider">Statut</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-content-muted uppercase tracking-wider">Responsable</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-content-muted uppercase tracking-wider">Dernière vérif.</th>
                <th className="px-6 py-4 text-right text-xs font-semibold text-content-muted uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-theme-border">
              {filteredEquipment.map((eq) => (
                <tr key={eq.id} className="hover:bg-theme-elevated transition-colors">
                  <td className="px-6 py-3">
                    <div>
                      <p className="text-sm font-medium text-content-primary">{eq.name}</p>
                      {eq.notes && <p className="text-xs text-content-subtle mt-0.5 line-clamp-1">{eq.notes}</p>}
                    </div>
                  </td>
                  <td className="px-6 py-3">
                    <CategoryChip
                  ...
                  <td className="px-6 py-3">
                    <code className="text-xs text-cyan-400 bg-cyan-400/10 px-2 py-1 rounded font-mono whitespace-nowrap block">{eq.qrCode}</code>
                  </td>
                  <td className="px-6 py-3">
                    <div>
                      <p className="text-sm text-content-primary">{eq.location}</p>
                      {eq.zone && <p className="text-xs text-content-subtle mt-0.5">{eq.zone}</p>}
                    </div>
                  </td>
                  <td className="px-6 py-3">
                  ...
                  <td className="px-6 py-3">
                    <p className="text-sm text-content-muted">{eq.lastCheck ? formatRelativeTime(eq.lastCheck) : '-'}</p>
                  </td>
      ...
      {showAddModal && (
        <AddEquipmentModal
          ...
          onAdd={async (newEquipment) => {
            const { id: _id, ...payload } = newEquipment;
