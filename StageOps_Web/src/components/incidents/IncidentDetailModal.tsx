    ...
    >
      ...
      >
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="p-4 bg-theme-elevated rounded-xl">
                ...
                <p className="text-sm text-content-primary">{formatRelativeTime(incident.timestamp)}</p>
              </div>
              {incident.resolvedAt && (
                <>
                  <div className="p-4 bg-theme-elevated rounded-xl">
                    <p className="text-[10px] text-content-subtle mb-1">Résolu le</p>
                    <p className="text-sm text-content-primary">{formatRelativeTime(incident.resolvedAt)}</p>
                  </div>
                  <div className="p-4 bg-theme-elevated rounded-xl">
                    <p className="text-[10px] text-content-subtle mb-1">Durée de résolution</p>
                    <p className="text-sm text-content-primary">
                      {Math.round((incident.resolvedAt.getTime() - incident.timestamp.getTime()) / 3600000)}h
                    </p>
                  </div>
                </>
              )}
          ...
          <div>
            <p className="text-xs text-content-subtle uppercase tracking-wider mb-3">Équipement concerné</p>
            <div className="flex items-center gap-4 p-4 bg-theme-elevated border border-theme-border rounded-xl">
              ...
              <div className="flex-1">
                <p className="text-sm text-content-primary">{equipmentName}</p>
                <p className="text-xs text-content-subtle mt-0.5">{equipmentLocation}</p>
              </div>
            </div>
          </div>
          ...
          {incident.resolutionNotes && (
            <div>
              <p className="text-xs text-content-subtle uppercase tracking-wider mb-3">Résolution</p>
              <div className="p-4 bg-green-500/5 border border-green-500/20 rounded-xl">
                <div className="flex items-center gap-2 mb-2">
                  <CheckCircle2 size={14} className="text-green-400" />
                  <p className="text-xs text-green-400">Résolu</p>
                </div>
                <p className="text-sm text-content-muted leading-relaxed">{incident.resolutionNotes}</p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="space-y-0">
              <TimelineItem color={sevColor} label={`Incident signalé par ${incident.reportedBy}`} time={formatRelativeTime(incident.timestamp)} />
              {incident.status === 'in-progress' && <TimelineItem color="#f59e0b" label="Prise en charge" time="En cours" />}
              {incident.resolvedAt && (
                <TimelineItem color="#22c55e" label="Incident résolu" time={formatRelativeTime(incident.resolvedAt)} />
              )}
            </div>
          </div>
